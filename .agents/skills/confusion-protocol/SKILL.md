...
Present structured decision briefs to the user. Use the agent's available user-input mechanism if available; otherwise present the brief directly in your response. Each question follows this format:

**Header** - one-line question title.

**Context** - identify the project, branch, and task context.

**Plain explanation** - 2-3 sentences a non-technical person could follow, naming what's at stake.
...
- **Neutral posture:** when there is no strong preference, say so but still keep the `(recommended)` label on the default.
