...
- Run theme architecture lint: `npm run lint:theme`
- Aggregate gate that includes theme architecture lint: `npm run lint`

`npm run lint:theme` uses these skill resources:
...
1. Run `npm run lint:theme` after Liquid, schema, theme JavaScript, Alpine, HTTP/cart, section refresh, or heading typography changes.
