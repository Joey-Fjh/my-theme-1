...
- Regenerate optimized icons: `npm run build:svg`

`npm run build:svg` uses this skill resource:
...
1. Run `npm run build:svg` only after `icons/*.svg` changes or when explicitly verifying the icon pipeline.
